...
fetch(`https://api.example.com/items?page=${page}&perPage=${perPage}`)
...
url = f"https://api.example.com/items?page={page}&perPage={perPage}"
...
func main() {
    page := 1      // Current page number
    perPage := 10  // Number of items per page
    url := fmt.Sprintf("https://api.example.com/items?page=%d&perPage=%d", page, perPage)
