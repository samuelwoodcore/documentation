---
title: Create Virtual Nuban
openapi: 'POST /{transmitta}/createnuban'
---

# Create Virtual Nuban

This endpoint is used to create a Virtual Nuban for a customer who is also created simultaneously. If `clientType` is cooperate, `businessName` is expected in the body of the request.
...
| productId | This is the `id` of the Savings product the `savingsAccountNo` of the customer would be attached to.  <br>  </br>`Must have cash accountling rule` configured. | Integer |
